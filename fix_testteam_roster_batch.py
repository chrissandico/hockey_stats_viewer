#!/usr/bin/env python3
"""
Script to fix the testteam roster issue by adding roster entries for completed games using batch operations.
"""
...
def fix_testteam_roster_batch():
    """Fix the testteam roster by adding entries for completed games using batch operations."""
    print("=== FIXING TESTTEAM ROSTER ISSUE (BATCH MODE) ===")
    ...
    game_roster = sheets_service.get_game_roster(force_refresh=True)  # Force refresh to get latest data
    ...
    try:
        ...
        # Prepare all roster entries in batches
        all_entries = []
        for game_id in games_needing_roster:
            print(f"Preparing roster entries for game {game_id}...")
            ...
            for _, player in test_players.iterrows():
                ...
                all_entries.append(roster_entry)
        
        print(f"\nPrepared {len(all_entries)} roster entries to add")
        
        # Add entries in smaller batches to avoid rate limits
        batch_size = 10
        total_batches = (len(all_entries) + batch_size - 1) // batch_size
        
        print(f"Adding entries in {total_batches} batches of {batch_size}...")
        ...
        for i in range(0, len(all_entries), batch_size):
            ...
            print(f"Adding batch {batch_num}/{total_batches} ({len(batch)} entries)...")
            
            # Add each entry in the batch
            for entry in batch:
                roster_worksheet.insert_row(entry, next_row)
                next_row += 1
                entries_added += 1
        ...
        print("\nVerifying additions...")
        time.sleep(3)  # Wait a bit before verification
...
if __name__ == "__main__":
    success = fix_testteam_roster_batch()
    if success:
        test_fix()
